...
export default function FileControl({refs, refcode, title, noExpire=false}){
    ...
    const handleEditFile = (id) => {
      fileService.fileGetList(id).then( res => {
        ...
        if( data.length > 0){
          ...
          setDes(`(แก้ไข) แนบไฟล์ สำหรับ item code : ${data[0]?.refcode || 'error'}`);
    ...
    const searchItems = [ 
      {
        key: '1',
        label: isReferent 
          ? <span><b>Referent</b> {`${refs} `} <DoubleRightOutlined /> {`code :  ${refcode}`}</span> 
        ...
        showArrow: false,
        extra: isReferent && <Button type="link" size="small" style={{lineHeight:1, fontWeight:400}} onClick={()=>{ setIsOpenUpload(true);  }}><UploadOutlined /> อัพโหลดไฟล์</Button>
      },
    ];       
    ...
    return !isOpenUpload ? (
      <Space direction="vertical" className='width-100 files-control' >
        ...
        <Card bordered={false}>
          <FileControlTable data={documentList} onEdit={(v)=>{ handleEditFile(v) }} noExpire={noExpire} ></FileControlTable> 
        </Card>
      </Space> 
    ) :
    (
